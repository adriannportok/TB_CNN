...
export default function AdminInicio() {
  ...
  const fetchData = async () => {
    try {
      ...
      const usuarios = Array.isArray(uRes.data)
        ? uRes.data.map((u) => ({
            ...
            nombre:
              u.nombres && u.apellidos
                ...
                : u.nombre ||
                  ...
                  "-",
            fechaRegistro:
              u.fecha_registro ?? u.fecha_creacion ?? u.created_at ?? null,
          }))
        : [];

      const pacientes = Array.isArray(pRes.data)
        ? pRes.data.map((p) => ({
            id: p.id_paciente,
      ...
      const analisisAnalizados = pacientes.filter(
        (p) => p.porcentaje !== null && typeof p.porcentaje !== "undefined"
      ).length;
      ...
      const usuariosRoles = Object.keys(roleCounts).map((k) => ({
        name: k,
        value: roleCounts[k],
      }));
      ...
      const eventos = [...recentUsers, ...recentPatients]
      ...
      const newUsers30d = usuarios.filter(
        (u) => u.fechaRegistro && new Date(u.fechaRegistro) >= days30
      ).length;
      const inactiveAccounts = usuarios.filter(
        (u) =>
          u.estado === false ||
          u.estado === "inactivo" ||
          u.estado === "Inactivo"
  ...
  return (
    <Layout title="Inicio de Administrador">
          ...
          <div className="p-6 bg-white rounded-2xl shadow-md border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Cuentas creadas en los últimos 30 días
                </p>
              </div>
              <div className="w-12 h-12 rounded-full bg-white/90 flex items-center justify-center ring-1 ring-gray-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="col-span-1 lg:col-span-2 bg-white rounded-2xl shadow-md p-6 border border-gray-100">
            <h4 className="text-lg font-semibold mb-4">
              Registro de Auditoría (Recientes)
            </h4>
            {!stats.eventos || stats.eventos.length === 0 ? (
              <div>
                ...
                <p className="text-xs text-gray-400 mt-2">
                  Nota: si su backend dispone de un endpoint de auditoría, puede
                  integrarlo para mostrar eventos de login y acciones de
                  usuario.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {stats.eventos.map((e, idx) => (
                  <div
                    key={idx}
                    className="flex items-start justify-between bg-gray-50 p-3 rounded-lg border"
                  >
                    <div>
